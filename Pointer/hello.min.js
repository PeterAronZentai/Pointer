...
if(obj&&obj.length>0)return false;if(obj&&obj.length===0)return true;for(var key in obj){if(obj.hasOwnProperty(key))return false;}
return true;}
...
else{return false;}},jsonp:function(p){if(p.method.toLowerCase()!=='get'&&!hello.utils.hasBinary(p.data)){p.data.method=p.method.toLowerCase();p.method='get';}},post:function(p){return{callbackonload:true};}}});
(function(){function formatItem(o){if(o.error){return;}
...
return true;}}});})();
hello.init({'yahoo':{oauth:{version:"1.0a",auth:"https://api.login.yahoo.com/oauth/v2/request_auth",request:'https://api.login.yahoo.com/oauth/v2/get_request_token',token:'https://api.login.yahoo.com/oauth/v2/get_token'},uri:{base:"http://social.yahooapis.com/v1/",me:"http://query.yahooapis.com/v1/yql?q=select%20*%20from%20social.profile%20where%20guid%3Dme&format=json","me/friends":'http://query.yahooapis.com/v1/yql?q=select%20*%20from%20social.contacts%20where%20guid=me&format=json'},wrap:{me:function(o){if(o.query&&o.query.results&&o.query.results.profile){o=o.query.results.profile;o.id=o.guid;o.name=o.givenName+' '+o.familyName;o.last_name=o.familyName;o.first_name=o.givenName;o.email=o.emails?o.emails.handle:null;o.thumbnail=o.image?o.image.imageUrl:null;}
return o;},"me/friends":function(o){var contact,field;o.data=o.query.results.contact;delete o.query;for(var i=0;i<o.data.length;i++){contact=o.data[i];o.data[i].id=null;for(var j=0;j<contact.fields.length;j++){field=contact.fields[j];if(field.type==='email'){o.data[i].email=field.value;}
if(field.type==='name'){o.data[i].first_name=field.value.givenName;o.data[i].last_name=field.value.familyName;o.data[i].name=field.value.givenName+' '+field.value.familyName;}
if(field.type==='yahooid'){o.data[i].id=field.value;}}}
return o;}},xhr:false}});
(function(){function formatUser(o){if(o.id){if(o.name){var m=o.name.split(" ");o.first_name=m[0];o.last_name=m[1];}
o.thumbnail=o.profile_image_url;}}
hello.init({'twitter':{oauth:{version:"1.0a",auth:"https://twitter.com/oauth/authorize",request:'https://twitter.com/oauth/request_token',token:'https://twitter.com/oauth/access_token'},uri:{base:"https://api.twitter.com/1.1/",me:'account/verify_credentials.json',"me/friends":'friends/list.json','me/share':function(p,callback){p.data={status:p.data.message};callback(p.method==='post'?'statuses/update.json?include_entities=1':'statuses/user_timeline.json');}},wrap:{me:function(o){formatUser(o);return o;},"me/friends":function(o){if(o.users){o.data=o.users;for(var i=0;i<o.data.length;i++){formatUser(o.data[i]);}
delete o.users;}
return o;},"me/share":function(o){if(!o.error&&"length"in o){return{data:o};}
return o;}},xhr:false}});})();
(function(){function formatUser(o){o.first_name=o.firstName;o.last_name=o.lastName;o.name=o.formattedName||(o.first_name+' '+o.last_name);o.thumbnail=o.pictureUrl;}
hello.init({'linkedin':{oauth:{version:"1.0a",auth:"https://api.linkedin.com/uas/oauth/authenticate",request:'https://api.linkedin.com/uas/oauth/requestToken?scope=r_fullprofile%2Br_emailaddress%2Br_network',token:'https://api.linkedin.com/uas/oauth/accessToken'},uri:{base:"https://api.linkedin.com/v1/",me:'people/~:(picture-url,first-name,last-name,id,formatted-name)',"me/friends":'people/~/connections'},scope:{basic:'r_fullprofile',email:'r_emailaddress',friends:'r_network'},wrap:{me:function(o){formatUser(o);return o;},"me/friends":function(o){if(o.values){o.data=o.values;for(var i=0;i<o.data.length;i++){formatUser(o.data[i]);}
delete o.values;}
return o;}},jsonp:function(p,qs){qs.format='jsonp';},xhr:false}});})();
hello.init({github:{name:'GitHub',oauth:{version:2,grant:'https://github.com/login/oauth/access_token'},uri:{auth:'https://github.com/login/oauth/authorize',base:'https://api.github.com/','me':'user','me/friends':'user/following'},wrap:{me:function(o){if(o.id){o.picture=o.avatar_url;o.thumbnail=o.avatar_url;o.name=o.login;}
return o;},"me/friends":function(o){if(Object.prototype.toString.call(o)==='[object Array]'){return{data:o};}
return o;}}}});
(function(){function format_file(o){if(typeof(o)!=='object'||"Blob"in window&&o instanceof Blob||"ArrayBuffer"in window&&o instanceof ArrayBuffer){return;}
var path=o.root+o.path.replace(/\&/g,'%26');if(o.thumb_exists){o.thumbnail=hello.settings.oauth_proxy+"?path="+encodeURIComponent('https://api-content.dropbox.com/1/thumbnails/'+path+'?format=jpeg&size=m')+'&access_token='+hello.getAuthResponse('dropbox').access_token;}
o.type=(o.is_dir?'folder':o.mime_type);o.name=o.path.replace(/.*\//g,'');if(o.is_dir){o.files='metadata/'+path;}
else{o.downloadLink=hello.settings.oauth_proxy+"?path="+encodeURIComponent('https://api-content.dropbox.com/1/files/'+path)+'&access_token='+hello.getAuthResponse('dropbox').access_token;o.file='https://api-content.dropbox.com/1/files/'+path;}}
hello.init({'dropbox':{oauth:{version:"1.0",auth:"https://www.dropbox.com/1/oauth/authorize",request:'https://api.dropbox.com/1/oauth/request_token',token:'https://api.dropbox.com/1/oauth/access_token'},uri:{base:"https://api.dropbox.com/1/",me:'account/info',"me/files":'metadata/dropbox',"default":function(p,callback){if(p.path.match("https://api-content.dropbox.com/1/files/")){if(p.method==='get'){p.method='blob';}}
callback(p.path);}},wrap:{me:function(o){if(!o.uid){return o;}
o.name=o.display_name;o.first_name=o.name.split(" ")[0];o.last_name=o.name.split(" ")[1];o.id=o.uid;delete o.uid;delete o.display_name;return o;},"default":function(o){if(o.is_dir){o.data=o.contents;delete o.contents;for(var i=0;i<o.data.length;i++){o.data[i].root=o.root;format_file(o.data[i]);}}
format_file(o);return o;}},xhr:function(p){if(p.path.match("https://api-content.dropbox.com/")){return true;}
return false;}}});})();